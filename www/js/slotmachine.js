...
function playSlotMachine() {
    slots = document.querySelector('#slotmachine');
    ...
    for (let col of cols) {
        ...
        for (let x = 0; x < 40 + (i * 3); x++) {
            ...
            let part = '<div class="icon" data-item="' + icon + '"><img src="img/slotmachine/' + icon + '.png"></div>';
            str += part
...
function setResult() {
    for (let col of cols) {
        ...
        for (let x = 0; x < 3; x++) {
            icon[x].setAttribute('src', 'img/slotmachine/' + results[x] + '.png');
            icon[(icon.length - 3) + x].setAttribute('src', 'img/slotmachine/' + results[x] + '.png');
        }
    }
}
